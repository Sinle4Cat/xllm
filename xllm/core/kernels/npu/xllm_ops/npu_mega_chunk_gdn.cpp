...
std::pair<torch::Tensor, torch::Tensor> npu_mega_chunk_gdn(
  ...
  if (cu_seqlens.has_value() && cu_seqlens->defined()) {
    ...
    if (!q_seq_lens.empty()) {
      ...
      VLOG(1) << "MegaChunkGdn sequence metadata source=host, sequences="
              << num_sequences << ", chunks=" << num_chunks;
    } else {
      VLOG(1) << "MegaChunkGdn sequence metadata source=device; copying "
                 "cu_seqlens to CPU.";
  ...
  auto a = zero_initialize_outputs
               ? torch::zeros({B, T, H, kMegaChunkSize}, opts_compute)
               : torch::empty({B, T, H, kMegaChunkSize}, opts_compute);
  ...
  auto h = zero_initialize_outputs
               ? torch::zeros({num_matrices, K, V}, opts_compute)
               : torch::empty({num_matrices, K, V}, opts_compute);
  ...
  auto final_state = zero_initialize_outputs
                         ? torch::zeros({num_sequences * H, K, V}, opts_compute)
                         : torch::empty({num_sequences * H, K, V}, opts_compute);
