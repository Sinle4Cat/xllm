...
torch::Tensor npu_mega_gdn_mtp_decode(const torch::Tensor& qkv,
  ...
  check_npu_tensor(num_accepted_tokens, "num_accepted_tokens");
  check_npu_tensor(norm_weight, "norm_weight");

  CHECK_EQ(qkv.dim(), 3) << "qkv must be [batch, sequence, channels].";
  CHECK_EQ(z.dim(), 4) << "z must be [batch, sequence, value_heads, 128].";
  CHECK_EQ(b.dim(), 3) << "b must be [batch, sequence, value_heads].";
  ...
  CHECK_EQ(z.sizes(),
           torch::IntArrayRef(
               {batch_size, sequence_length, num_value_heads, kHeadDim}));
  CHECK_EQ(b.sizes(),
           torch::IntArrayRef({batch_size, sequence_length, num_value_heads}));
