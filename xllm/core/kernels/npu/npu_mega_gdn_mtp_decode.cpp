...
constexpr int64_t kMinSpeculativeTokens = 1;
constexpr int64_t kMaxSpeculativeTokens = 16;

bool is_supported_speculative_tokens(int64_t speculative_tokens) {
  return speculative_tokens >= kMinSpeculativeTokens &&
         speculative_tokens <= kMaxSpeculativeTokens;
}

void check_dtype_and_contiguous(const torch::Tensor& tensor,
                                torch::ScalarType dtype,
                                const torch::Device& device,
                                const char* name) {
  check_tensor(tensor, name, "mega_gdn_mtp_decode");
  CHECK_EQ(tensor.scalar_type(), dtype)
      << "mega_gdn_mtp_decode: " << name << " has an invalid dtype";
  CHECK_EQ(tensor.device().type(), c10::DeviceType::PrivateUse1)
      << "mega_gdn_mtp_decode: " << name << " must be on NPU";
  CHECK_EQ(tensor.device(), device)
      << "mega_gdn_mtp_decode: all tensors must share an NPU device";
  CHECK(tensor.is_contiguous())
      << "mega_gdn_mtp_decode: " << name << " must be contiguous";
}
...
torch::Tensor mega_gdn_mtp_decode(const torch::Tensor& qkv,
  ...
  check_dtype_and_contiguous(z, torch::kBFloat16, device, "z");
  check_dtype_and_contiguous(b, torch::kBFloat16, device, "b");
  check_dtype_and_contiguous(a, torch::kBFloat16, device, "a");
  check_dtype_and_contiguous(
      conv_weight, torch::kBFloat16, device, "conv_weight");
  check_dtype_and_contiguous(
      conv_state, torch::kBFloat16, device, "conv_state");
  check_dtype_and_contiguous(a_log, torch::kFloat32, device, "a_log");
  check_dtype_and_contiguous(dt_bias, torch::kFloat32, device, "dt_bias");
  check_dtype_and_contiguous(ssm_state, torch::kFloat32, device, "ssm_state");
  check_dtype_and_contiguous(
      read_state_indices, torch::kInt32, device, "read_state_indices");
  check_dtype_and_contiguous(
      write_state_indices, torch::kInt32, device, "write_state_indices");
  check_dtype_and_contiguous(
      num_accepted_tokens, torch::kInt32, device, "num_accepted_tokens");
  check_dtype_and_contiguous(
      norm_weight, torch::kBFloat16, device, "norm_weight");
  ...
  const int64_t sequence_length = qkv.size(1);
  const int64_t speculative_tokens = sequence_length - 1;
  CHECK(is_supported_speculative_tokens(speculative_tokens))
      << "mega_gdn_mtp_decode: unsupported K=" << speculative_tokens;
  ...
  const int64_t num_v_heads = z.size(2);
  ...
  if (std::getenv("XLLM_USE_TILELANG_MTP_SEGMENTED") != nullptr &&
      tilelang::has_mega_gdn_mtp_decode_segmented_specialization(
          ...
          num_v_heads,
          speculative_tokens)) {
    return tilelang::mega_gdn_mtp_decode_segmented(qkv,
                                                   z,
                                                   b,
                                                   a,
                                                   conv_weight,
